...
export default function ListingDetail() {
  ...
  // Direct WhatsApp link (sidebar shortcut, uses default message)
  const quickWhatsAppLink = `https://wa.me/${listing.landlord.phone.replace(/[^\d]/g, "")}?text=${encodeURIComponent(defaultMessage)}`
          ...
          <section className={styles.main}>
            ...
            <div className={styles.body}>
              <div className={styles.statsRow}>
                <span><BedDouble size={17} /> {listing.beds} bed</span>
                <span><Bath size={17} /> {listing.baths} bath</span>
                <span><Ruler size={17} /> {listing.sqm} sqm</span>
                <span><Star size={17} /> {listing.rating} ({listing.reviews} reviews)</span>
              </div>
          ...
          <div className={styles.modal}>
            ...
            {sent ? (
              <div className={styles.success}>
                <CheckCircle size={42} />
                <p>WhatsApp opened! Continue the conversation with <strong>{listing.landlord.name}</strong> there.</p>
                <button
              ...
              <>
                <p style={{ marginBottom: 16, color: "var(--color-text-muted)", fontSize: 14 }}>
                  Your message will be sent to <strong>{listing.landlord.name}</strong> via WhatsApp.
                </p>
                ...
                <div className={styles.formField}>
                  <label>
                    <Calendar size={14} />
                    Preferred move-in date <span style={{ color: "var(--color-text-muted)", fontWeight: 400 }}>(optional)</span>
                  </label>
                  <input
                    type="date"
                    className="input"
